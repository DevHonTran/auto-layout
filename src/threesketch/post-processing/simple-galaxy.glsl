...
float Noise2D( in vec2 x )
{
    ...
    f = f*f*(3.0-2.0*f);
    ivec2 uv = p.xy;
    float rgA = texelFetch( iChannel, (uv+ivec2(0,0))&255, 0 ).x;
    float rgB = texelFetch( iChannel, (uv+ivec2(1,0))&255, 0 ).x;
    float rgC = texelFetch( iChannel, (uv+ivec2(0,1))&255, 0 ).x;
    float rgD = texelFetch( iChannel, (uv+ivec2(1,1))&255, 0 ).x;
...
void main( )
{
    ...
    //vec4 finalColor = vec4(BackgroundColor(gl_FragCoord.xy * 0.25), 1.0);
    //vec4 finalColor = vec4(l1Col * noiseVal, 1.0);
    finalColor = vec4(mix(BackgroundColor(gl_FragCoord.xy * 0.125), l1Col * noiseVal, pow(noiseVal, 1.0)), 1.0);
...
//    fragColor = finalColor;
    gl_FragColor = texture(tDiffuse, vUv) + finalColor;
}